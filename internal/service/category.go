...
func (c *CategoryService) UpdateCategorySteamBidirectional(stream pb.CategoryService_UpdateCategorySteamBidirectionalServer) error {
	for {
		category, err := stream.Recv()
		if err == io.EOF {
			return nil
		...
		categoryResult, err := c.CategoryDB.UpdateItem(category.Id, category.Name, category.Description)
		if err != nil {
			return status.Errorf(codes.Internal, "failed to update category: %v", err)
		}
