...
func (c *Category) UpdateItem(id, name, description string) (Category, error) {
	_, err := c.db.Exec("UPDATE categories SET name = ?, description = ? WHERE id = ?", name, description, id)
	if err != nil {
		return Category{}, err
	}
	return Category{ID: id, Name: name, Description: description}, nil
